...
const url = 'https://raw.githubusercontent.com/AzurAPI/azurapi-js-setup/master/ships.json';

rp(url)
  .then(function(html){
    //success!
    let ships = JSON.parse(html);

    // fs.writeFile('js/ship_data.js', html, err => {
    // if (err) {
    //   console.error(err)
    //   return
    // }
    ...
    for (i in ships){
      ...
      if (s.retrofit !== undefined){
        ...
        out[`${id}`].painting = `https://raw.githubusercontent.com/x94fujo6rpg/AzurLaneFleet/master/shipicon/${shipName}.png`;
