...
const ship_json = JSON.parse(fs.readFileSync('./data/ship_data.json').toString())
const equip_json = JSON.parse(fs.readFileSync('./data/gear_data.json').toString())
...
const ship_popup = new Vue({
    ...
    <div id="shiplist" class="container-fluid row m-0 p-0" type="-" nat="-" rarity="-" NS="" >
      ...
      <button v-for="ship in ships" class="p-1 item_container tosort" :type="ship.type" :nat="ship.nationality" :rarity="ship.rarity" :id="ship.id" onclick="setShipAndEquip(this)" data-dismiss="modal">
        <div class="container-fluid p-0 box">
          <div class="icon_box row"><img class="img-fluid icon" loading="lazy" :src="ship.painting" onError="this.onerror=null;this.src='https://raw.githubusercontent.com/Drakomire/perseus-data/master/AzurLaneImages/assets/artresource/atlas/squareicon/unknown.png';">
